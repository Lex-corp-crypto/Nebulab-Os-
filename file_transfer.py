...
class FileTransferManager:
    ...
    def _get_safe_path(self, filename: str) -> Optional[str]:
        """Résout et sécurise le chemin du fichier pour empêcher toute traversée de répertoire"""
        safe_name = os.path.basename(filename).strip()
        if not safe_name or safe_name in [".", ".."]:
            return None
        full_path = os.path.abspath(os.path.join(self.storage_dir, safe_name))
    ...
    async def delete_file(self, filename: str) -> bool:
        """Supprime un fichier du stockage partagé en toute sécurité"""
        path = self._get_safe_path(filename)
        if not path or not os.path.exists(path):
            return False
        try:
            os.remove(path)
            logger.info(f"Fichier supprimé du stockage partagé : {filename}")
            return True
        except Exception as e:
